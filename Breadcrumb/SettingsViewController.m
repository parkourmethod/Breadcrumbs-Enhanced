...
- (void)configureWithOptions:(AccuracyPickerOption *)options
{
    ...
    PKPositionTrackingMode accuracy = [accuracyNum doubleValue];
    ...
    else if (accuracy == pkAutomotive)
    {
        row = 1;
    }
    else if (accuracy == pkFitness)
    {
        row = 2;
    }
    else if (accuracy == pkPedestrian)
    {
        row = 3;
    }
    else if (accuracy == pkGeofencing)
    {
        row = 4;
    }
    else if (accuracy == pkLowEnergy)
    {
        row = 5;
    }
...
//    CLLocationAccuracy accuracyValue = -1;
    PKPositionTrackingMode accuracyValue = -1;
    
    switch (row)
    {
        case 0:
            title = @"pkShare";
            accuracyValue = pkShare;
            break;
        case 1:
            title = @"pkAutomotive";
            accuracyValue = pkAutomotive;
            break;
        case 2:
            title = @"pkFitness";
            accuracyValue = pkFitness;
            break;
        case 3:
            title = @"pkPedestrian";
            accuracyValue = pkPedestrian;
            break;
        case 4:
            title = @"pkGeofencing";
            accuracyValue = pkGeofencing;
            break;
        case 5:
            title = @"pkLowEnergy";
